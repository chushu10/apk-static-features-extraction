...
def get_cert_info(id, database):
    ...
    if CERT_INFO is None:
        return '', ''
    serial = re.search('SerialNumber: \[(.+?)\]', CERT_INFO[0])
    issuer = re.search('</br>  Issuer: (.+?)</br>', CERT_INFO[0])
    if serial and issuer:
        return serial.group(1).strip(), issuer.group(1), CERT_INFO[1]
    else:
        return '', '', CERT_INFO[1]
...
def get_serial_name_dict(serial_set, serial_name, database_path):
    ...
    for i in range(1, MAX_ID + 1):
        SerialNumber, Issuer, APP_NAME = get_cert_info(i, database_path)
        SerialIssuer = SerialNumber + ':' + Issuer
        if SerialNumber == '' or Issuer == '' or APP_NAME == '':
            continue
        print('Scanning app %d' % i)
        if SerialIssuer not in serial_set:
            serial_set.add(SerialIssuer)
            serial_name[SerialIssuer] = dict(appnames=[])
            serial_name[SerialIssuer]['appnames'].append(APP_NAME)
        else:
            serial_name[SerialIssuer]['appnames'].append(APP_NAME)
...
def main():
    ...
    print('%d serial issuer sign %d apps' % (len(serial_name), TOTAL))

    f = open(os.path.join(directory, 'serial_issuer.json'), 'w')
